...
const MESSAGES = {
  zh: {
    ...
    "pwd.disable_requires_hidden_off": "请先关闭“使用隐藏消息代替锁屏密码”，再关闭密码保护",

    "hidden.toggle_label": "使用隐藏消息代替锁屏密码（数据也会被加密）",
    "hidden.toggle_hint": "开启后，打开应用不再显示锁屏密码页面；被标记为隐藏的对话会在空闲锁定或重新打开应用时从列表中消失（仍以加密形式保存），需要在对话搜索框上连续点击三下并输入密码才能找回。关闭此开关需要先输入当前密码解锁所有隐藏对话。",
    "hidden.enabled_msg": "隐藏消息模式已启用",
    "hidden.disabled_msg": "隐藏消息模式已关闭，所有隐藏对话已恢复",
    "hidden.enter_password_to_disable": "请先在上方输入当前密码",
    "hidden.hide_title": "隐藏此对话",
    "hidden.unhide_title": "取消隐藏此对话",
    "hidden.unlock_title": "解锁隐藏对话",
    "hidden.unlock_password_label": "密码",
    "hidden.unlock_confirm": "解锁",
  ...
  en: {
    ...
    "pwd.disable_requires_hidden_off": "Turn off \"Use hidden messages instead of a lock screen\" first, then disable password protection",

    "hidden.toggle_label": "Use hidden messages instead of a lock screen (data is still encrypted)",
    "hidden.toggle_hint": "When on, opening the app no longer shows a lock screen; chats flagged hidden disappear from the list (still stored encrypted) whenever the idle-lock timer fires or the app is reopened — triple-click the chat search box and enter your password to bring them back. Turning this off requires entering your current password to unlock everything first.",
    "hidden.enabled_msg": "Hidden messages mode enabled",
    "hidden.disabled_msg": "Hidden messages mode disabled — all hidden chats restored",
    "hidden.enter_password_to_disable": "Enter your current password above first",
    "hidden.hide_title": "Hide this chat",
    "hidden.unhide_title": "Unhide this chat",
    "hidden.unlock_title": "Unlock hidden chats",
    "hidden.unlock_password_label": "Password",
    "hidden.unlock_confirm": "Unlock",
